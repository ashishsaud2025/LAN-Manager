...
class NetworkTopology(QWidget):
    ...
    def _add_node(self, x: float, y: float, title: str, detail: str,
        ...
        diameter = 104.0 if local else 90.0
        fill = QColor(colors["accent"] if local else colors["raised"])
        ...
        title_item.setDefaultTextColor(QColor(
            colors["on_accent"] if local else colors["ink"]))
        title_item.setFont(QFont(title_item.font().family(), 9, QFont.Weight.Bold))
        title_bounds = title_item.boundingRect()
        title_item.setPos(-title_bounds.width() / 2, -22)
        detail_item = self.scene.addText(detail)
        detail_item.setDefaultTextColor(QColor(
            colors["on_accent"] if local else colors["secondary"]))
        detail_item.setFont(QFont(detail_item.font().family(), 8))
        detail_bounds = detail_item.boundingRect()
        detail_item.setPos(-detail_bounds.width() / 2, 2)
        group = self.scene.createItemGroup([ellipse, title_item, detail_item])
        group.setPos(x, y)
